...
- [x] Google Sheets Q&A (`app/integrations/sheets.py` — CSV-export trick for public/shared links, no OAuth needed; bound into `agent.py`'s tool dispatch). Known limitation: exact counting/math over sheet data can be slightly off since there's no computation layer, just raw text in context — acceptable per locked no-RAG/no-embeddings approach, fine for qualitative questions
- [x] PDF upload Q&A (`app/integrations/documents.py` using pypdf — text extracted from uploaded PDF, injected into that turn only; DB history keeps a short placeholder instead of the full extracted text so future turns don't replay the whole document)
